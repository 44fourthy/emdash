 ...
 */
export function hostAccountLockForProject(
  projectId: string,
  accounts: LockableAccount[]
): HostAccountLock | undefined {
  ...
  // The machine record has not loaded; resolve to nothing rather than guess.
  if (!connection) return { accountId: null, providerId: GITHUB_PROVIDER_ID };
  return hostAccountLock({
    host: connection.host,
    username: connection.username,
    storedAccountId: connection.githubAccountId,
    accounts,
  });
}
